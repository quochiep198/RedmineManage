...
export default function IssueSyncPage() {
  ...
  useEffect(() => {
    async function loadStatus() {
      try {
        const res = await api.get("/issues/sync/status");
        setStatus(res.data);
      } catch (err) {
        setError(err.response?.data?.detail || "Could not load issue sync status.");
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleSync() {
    ...
    try {
      ...
      setNotice(`Synced ${res.data.total_synced} issues successfully.`);
    } catch (err) {
      ...
      if (err.code === "ECONNABORTED") {
        ...
        setError(data?.message || data?.detail || "Issue sync failed.");
      }
    } finally {
      setSyncing(false);
    }
  ...
  if (loading) {
    return <div className="panel">Loading issue sync status...</div>;
  }
    ...
    <section className="connection-layout">
      <div className="panel connection-panel">
        ...
        <p className="muted-copy">
          Sync all issues for the current Redmine project using the shared connection and stored
          project identifier.
        </p>
        ...
        <div className="meta-list project-sync-metrics">
          ...
          <div className="meta-row">
            ...
            <strong>{status?.project_identifier ?? "Not configured"}</strong>
          </div>
          <div className="meta-row">
            <span>Current project</span>
            <strong>{status?.project_name ?? "Project not synced yet"}</strong>
          </div>
          <div className="meta-row">
            <span>Last sync</span>
